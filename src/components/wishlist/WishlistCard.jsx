...
{wishlistData.map((item)=>{
    return    <article key={item.id} className='p-1 border  flex rounded-lg  gap-4 flex-col md:flex-row'>
    <Link to={`/edit-course/${item.id}`} className='bg-slate-100 rounded-lg h-36 md:h-auto'>

        <img src={item.image} alt="" className='rounded h-full w-full object-cover object-left' />
    </Link>
    ...
    <Link to={`/edit-course/${item.id}`} className='p-2 flex flex-col gap-2 w-full'>
        <h2 className='font-[700] text-[#31225C
] text-[18px]'>{item.title}</h2>
        <div className='flex gap-4 items-center'>
            <span className='text-[#31225C] font-[700] text-[14px]'>{item.rating}</span>
...
{[...Array(5-Math.floor(item.rating))].map(()=>{
  ...
  return  <li key={id}>
    <img src={emptystar} alt="star" />
            ...
            <span className='text-[#31225C] font-[400] text-[14px]'>({item.totalRating})</span>
        </div>
        ...
        <p className='text-primary text-sm font-bold'>Available Credit : <span>{item.credit}</span>
        </p>
    
        <p className='text-[
#525252] text-[13px] font-[400] sm:w-[90%]'>{item.desc.substring(0, 200)}...</p>
...
<button  className='primary-btn '>Move to cart
